...
namespace gw2f {
    namespace bxml2 {
        ...
        namespace
        {
            ...
            bool outputElement( const XmlElement& p_element, std::ostream& p_stream, size_t p_depth )
            {
                ...
                // attributes
                for ( size_t i = 0; i < p_element.attributeCount( ); i++ )
                {
                    p_stream
                        << " "
        ...
        size_t XmlWriter::write( const XmlDocument& p_document, byte* p_buffer, size_t p_maxSize )
        {
            std::stringstream stream;
            write( p_document, stream );

            auto size = std::min( static_cast<size_t>( stream.tellg( ) ), p_maxSize );
